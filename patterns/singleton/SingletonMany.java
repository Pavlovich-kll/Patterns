...
public final class SingletonMany {
    // Поле обязательно должно быть объявлено volatile, чтобы двойная проверка
    // блокировки сработала как надо.
    private static volatile SingletonMany instance;

    public String value;

    private SingletonMany(String value) {
        this.value = value;
    }

    public static SingletonMany getInstance(String value) {
        ...
        SingletonMany result = instance;
        if (result != null) {
            return result;
        }
        synchronized(SingletonMany.class) {
            if (instance == null) {
                instance = new SingletonMany(value);
            }
            return instance;
        }
    }
